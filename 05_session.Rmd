...
The thing I want you to focus on this week is writing functions to calculate covariance and correlation, this time using `dplyr` functions and pipes (`%>%`). In the functions I have created, I use `mutate()`, `summarize()`, and `select()`, but other combinations of functions could also work. You will also need to use the function `nrow()` to count the number of observations (N in the equation on the website), as well as `mean()` and `sum()`. 

One important difference from how these functions are created on the website, is that I want you to make the functions a bit more flexible, by allowing users to input both the data they want to use as well as the to columns to use for the calculations (e.g. `my_covariance <- function(data, x, y){}`). There is an issue with specifying column names when running "homemade" functions and a trick (shown in the code chunk below) will be needed. For the function to successfully "send" the input for column names, this part will have to be included within *double* curly brackets (`{{}}`) when specified in the "body" of the function: 
